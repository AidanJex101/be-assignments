...
def users_get_by_search():
    ...
    user_data = db.session.query(AppUsers).filter(db.or_(
        db.func.lower(AppUsers.name).contains(search_term), 
        db.func.lower(AppUsers.email).contains(search_term),
        db.func.lower(AppUsers.last_name).contains(search_term)
        )).order_by(AppUsers.last_name.asc()).all()
